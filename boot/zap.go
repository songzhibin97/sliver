...
func InitZap() {
	if ok, _ := utils.PathExists(global.Config.Zap.Director); !ok { // 判断是否有Director文件夹
		fmt.Println(global.I18n.TranslateFormat(`{#CreateDirectory} %v`, global.Config.Zap.Director))
	...
	if err != nil {
		fmt.Println(global.I18n.TranslateFormat(`{#GetWriteSyncer} %v`, err))
		return
	}

	if level == zap.DebugLevel || level == zap.ErrorLevel {
		global.Zap = zap.New(getEncoderCore(), zap.AddStacktrace(level))
	} else {
		global.Zap = zap.New(getEncoderCore())
	}
	if global.Config.Zap.ShowLine {
		global.Zap = global.Zap.WithOptions(zap.AddCaller())
	}
}
